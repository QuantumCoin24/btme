...
function requireSupabase() {
  if (!isSupabaseConfigured) {
    throw new Error(
      'BTME messaging is not configured.'
    )
  }
...
async function getAuthenticatedMemberId() {
  ...
  if (!user) {
    throw new Error(
      'You must be signed in to use Spark™.'
    )
  }
...
export async function sendSparkMessage(
  ...
  if (!cleanConversationId) {
    throw new Error(
      'This Spark™ conversation is not available.'
    )
  }
  ...
  if (cleanBody.length > 4000) {
    throw new Error(
      'Spark™ messages can be up to 4,000 characters.'
    )
  }
    ...
    supabase
      ...
      .on(
        'postgres_changes',
        {
          ...
          table: 'messages',
          filter:
            `conversation_id=eq.${cleanConversationId}`
        },
        (payload) => {
          if (!active) {
            return
      ...
      .subscribe(
        (status) => {
          if (
            ...
            onError(
              'Realtime Spark™ connection was interrupted.'
            )
          }
        }
      )
