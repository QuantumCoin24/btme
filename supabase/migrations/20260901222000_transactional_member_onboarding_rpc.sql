...
begin
  ...
  if char_length(btrim(p_first_name)) < 2 then
  ...
  if char_length(btrim(p_city)) < 2 then
  ...
  if coalesce(array_length(p_lifestyle_signals, 1), 0) < 2 then
    raise exception 'Lifestyle profile is incomplete.';
  end if;

  if char_length(btrim(p_perfect_sunday)) < 10 then
    raise exception 'Perfect Sunday answer is incomplete.';
  end if;

  if char_length(btrim(p_green_flag)) < 6 then
    raise exception 'Green flag answer is incomplete.';
  end if;

  if char_length(btrim(p_absolute_no)) < 6 then
...
revoke all
on function public.complete_member_onboarding(
  text,
  date,
  text,
  text,
