-- Align Apple subscription authority with the accepted entitlement actor enum.
-- Production failure discovered during Build 12 restore testing:
-- apply_verified_apple_subscription supplied "apple_app_store" as actor_type,
-- while private.set_member_entitlement_authority accepts "app_store".
...
begin
  ...
  if existing_member_id is not null
    ...
    raise exception
      'Apple subscription is already bound to another member';
  end if;

  if existing_signed_date is not null
     and p_signed_date < existing_signed_date then
    raise exception
      'stale Apple subscription transaction rejected';
  end if;

  if exists (
    select 1
    from private.apple_subscription_bindings
    where latest_transaction_id =
      p_latest_transaction_id
      and original_transaction_id <>
        p_original_transaction_id
  ) then
    raise exception
      'Apple transaction replay detected';
  end if;
  ...
  do update set
    latest_transaction_id =
      excluded.latest_transaction_id,
    product_id =
      excluded.product_id,
    environment =
    ...
    entitlement_status =
      excluded.entitlement_status,
    purchase_date =
      excluded.purchase_date,
    signed_date =
      excluded.signed_date,
    expires_at =
      excluded.expires_at,
    revocation_date =
      excluded.revocation_date,
    last_verified_at =
      now(),
    updated_at =
      now()
  where private.apple_subscription_bindings.member_id =
        excluded.member_id
    and excluded.signed_date >=
        private.apple_subscription_bindings.signed_date;
